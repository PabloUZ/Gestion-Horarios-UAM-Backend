...
class CourseApprovedRepository():
    ...
    def get_course_approved(self, course_approved_id:int) -> CoursesApproved:
        course_approved = self.db.query(CoursesApprovedModel).filter(CoursesApprovedModel.id == course_approved_id).first()
    ...
    def update_course_approved(self, course_approved_id:int, course_approved:CoursesApproved) -> CoursesApproved:
        upd_course_approved = self.db.query(CoursesApprovedModel).filter(CoursesApprovedModel.id == course_approved_id).first()
        self.db.commit()
        self.db.refresh(upd_course_approved)
        return upd_course_approved
    
    def delete_course_approved(self, course_approved_id:int) -> dict:
        del_course_approved = self.db.query(CoursesApprovedModel).filter(CoursesApprovedModel.id == course_approved_id).first()
